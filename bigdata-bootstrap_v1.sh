...
# Guarded, idempotent, with robust downloads (strong checksum parsing + aria2 fallback)
# ===============================
...
CACHE_DIR="${HOME}/bigdata-cache"

# ---- UI helpers ----
RED="$(printf '[31m')"; GREEN="$(printf '[32m')"; YELLOW="$(printf '[33m')"; BLUE="$(printf '[34m')"; NC="$(printf '[0m')"
msg(){ printf "%b
" "${BLUE}==>${NC} $*"; }
ok(){ printf "%b
" "${GREEN}✔${NC} $*"; }
  ...
  for u in $urls; do
    ...
    if _download_url "$u" "$out.part"; then
      ...
      if tar -tf "$out.part" >/dev/null 2>&1; then
        ...
        echo "Archive invalid from this mirror, next…"
      fi
    else
...
preflight(){
  ...
  sudo apt-get install -y curl wget tar gzip unzip jq iproute2 util-linux ca-certificates \
                           python3 python3-venv python3-pip openssh-server aria2 \
  ...
  ensure_dir "$DEV_DIR" "$HDFS_NAME" "$HDFS_DATA" "$ZK_DATA" "$KAFKA_DATA" "$AIRFLOW_HOME" "$HOME/hbasedata" "$CACHE_DIR"
